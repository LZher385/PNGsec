...
impl Png {
    ...
    fn as_bytes(&self) -> Vec<u8> {
        ...
        let chunks_bytes_vec: Vec<u8> = self
            ...
            .flatten()
            .collect();
        result.extend(chunks_bytes_vec);
        result
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        while offset < value.len() {
            let length = u32::from_be_bytes(value[offset..(offset + 4)].try_into()?);
            let chunk_bytes = &value[offset..(offset + length as usize + 12)];
            chunks.push(Chunk::try_from(chunk_bytes)?);
            offset += length as usize + 12;
        }
        if offset != value.len() {
